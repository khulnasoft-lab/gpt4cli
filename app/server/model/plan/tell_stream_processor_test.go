...
func TestBufferOrStream(t *testing.T) {
	tests := []struct {
...

		{
			...
			chunk:        "hello <Gpt4cliFinish/>bye",
			manualStop:   []string{"<Gpt4cliFinish/>"},
			want: bufferOrStreamResult{
		...
		},
		{
			...
			chunk:      "<Gpt4cliFin", // no '<' in second part
			manualStop: []string{"<Gpt4cliFinish/>"},
			want: bufferOrStreamResult{
			...
			wantState: &chunkProcessor{
				contentBuffer: "<Gpt4cliFin", // prefix is buffered
			},
		},
		{
			...
			initialState: &chunkProcessor{
				contentBuffer: "<Gpt4cliFin", // leftover from previous call
			},
